...
function buildWrapperScript(codexDir) {
  if (IS_WINDOWS) {
    // Use Node.js wrapper on Windows to avoid PowerShell's -i parameter parsing issue
    // PowerShell interprets -i as -InformationAction, breaking Docker's interactive flag
    const wrapperPath = path.join(codexDir, 'docker-wrapper.js');
    const content = [
      '#!/usr/bin/env node',
      '// Docker wrapper for dynamic workspace mounting in Codex MCP',
      '// Uses Node.js on Windows to avoid PowerShell parameter parsing issues',
      '',
      'const { spawn } = require("child_process");',
      'const path = require("path");',
      'const fs = require("fs");',
      '',
      '// Get workspace directory from PWD env or current directory',
      'const workspaceDir = process.env.PWD || process.cwd();',
      '',
      '// Convert Windows path to Docker path format (C:\\foo -> /c/foo)',
      'function toDockerPath(winPath) {',
      '    if (!winPath) return "/workspace";',
      ...
      '}',
      '',
      'const dockerPath = toDockerPath(workspaceDir);',
      '',
      ...
      '',
      '// Process arguments - replace PLACEHOLDER and inject HOST_WORKSPACE',
      ...
      '    if (arg === "run" && !foundRun) {',
      '        newArgs.push("run");',
      '        newArgs.push("-e", `HOST_WORKSPACE=${workspaceDir}`);',
      '        foundRun = true;',
      '        continue;',
      '    }',
      ...
      '        newArgs.push("-v");',
      '        const volArg = args[++i];',
      '        if (volArg.includes("PLACEHOLDER")) {',
      '            newArgs.push(`${dockerPath}:/workspace:ro`);',
      '        } else {',
      '            newArgs.push(volArg);',
      '        }',
      '        continue;',
      ...
      '}',
      '',
      '// Spawn docker with stdin/stdout/stderr inheritance',
      'const child = spawn(dockerBin, newArgs, {',
      '    stdio: "inherit",',
      '    shell: false',
      '});',
      '',
      'child.on("error", (err) => {',
      '    process.stderr.write(`Failed to start docker: ${err.message}\\n`);',
      '    process.exit(1);',
      '});',
      '',
      'child.on("exit", (code) => {',
  ...
  const content = [
    ...
    '    ARGS+=("${WORKSPACE_DIR}:/workspace:ro")',
    '  elif [[ "$arg" == "run" ]] && [ "$INJECT_HOST_WORKSPACE" = true ]; then',
    '    # After "run", inject HOST_WORKSPACE env var',
    '    ARGS+=("$arg")',
    '    ARGS+=("-e" "HOST_WORKSPACE=${WORKSPACE_DIR}")',
    '    INJECT_HOST_WORKSPACE=false',
    '  else',
    '    ARGS+=("$arg")',
    '  fi',
    'done',
